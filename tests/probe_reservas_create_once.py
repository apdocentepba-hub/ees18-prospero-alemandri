...
MARKER = "PRUEBA E2E ESTABLE 2026-09-01 17:37"
EMAIL = "secundaria18avellaneda@abc.gob.ar"


def get_json(params, timeout=60):
    url = BASE_URL + "?" + urllib.parse.urlencode(params)
    req = urllib.request.Request(url, headers={"User-Agent": "EES18-reservas-create-once/1.0"})
...
def choose_slot():
    for year, month in [(2026, 9), (2026, 10)]:
        month_data = get_json({"action": "month", "year": str(year), "month": str(month)}, timeout=40)
...
payload = {
    ...
    "subject": MARKER,
    "resources": {},
    "observations": "Reserva técnica controlada. Será retirada al terminar la verificación."
}
started = time.perf_counter()
result = get_json({"action": "create", "payload": json.dumps(payload, ensure_ascii=False)}, timeout=75)
